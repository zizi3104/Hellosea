...
export function validateInquiry(body, now = new Date(), currentPlans = plans) {
  ...
  const name = text('name'), email = text('email').toLowerCase(), phone = text('phone'), message = text('message'), preferredDate = text('preferredDate'), preferredTime = text('preferredTime'), level = text('level');
  ...
  if (preferredTime && !/^(?:0[5-9]|1[0-6]):00$/.test(preferredTime)) return { error:'Please choose a time from 05:00 through 16:00 in Lombok time.' };
  ...
  const plan=currentPlans.find(p=>p.id===planKey);
  if(planKey&&!plan)return {error:'Please choose a valid lesson option.'};
  const savedDetails=[];
  if(preferredTime)savedDetails.push(`[Preferred time, Lombok] ${preferredTime}`);
  if(plan)savedDetails.push(`[Reference only, not a quote] ${plan.title.en}: IDR ${plan.price}/person; ${body.people} surfers; total IDR ${plan.price*body.people}.`);
  const savedMessage=[...savedDetails,message].filter(Boolean).join('\n');
