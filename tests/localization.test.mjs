...
const dictionary = vm.runInNewContext(i18n.slice(0,i18n.indexOf("let language ="))+'translations');
test('Every visible translation and dynamic state has all three language versions',()=>{
 ...
 const dynamic=[...app.matchAll(/t\('([^']+)'\)/g)].map(m=>m[1]);
...
test('Language switches update locale and metadata without replacing form controls',()=>{
 ...
 const context=vm.createContext({localStorage:{getItem:()=>null,setItem(){}},Event:class{constructor(type){this.type=type;}},document:{documentElement:{lang:'en'},querySelector:()=>meta,querySelectorAll:sel=>sel==='[data-lang]'?buttons:sel==='[data-i18n]'?nodes:[],dispatchEvent(){notifications++;},fields}});
 vm.runInContext(i18n,context);
 assert.equal(context.document.documentElement.lang,'en');
 vm.runInContext("setLanguage('ja')",context);assert.equal(context.document.documentElement.lang,'ja');assert.equal(nodes[0].innerHTML,'お名前');
